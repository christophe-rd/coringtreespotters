...
Previous year climate also had limited effects on growth. For the one species that showed decreased growth with warmer thermal season, this effect remained for the current year, but previous year GDD actually increased growth for {\Ball}
...
\Sexpr{bspp_df2_ts_prvs$p95[bspp_df2_ts_prvs$spp_name %in% "B. alleghaniensis"]} RW/GDD). {\Ball}'s congeneric {\Bnig}, also grew less with previous year GDD ($\beta =$ 
...
\Sexpr{sprintf("%.2f", bspp_df2_ts_prvs$p95[bspp_df2_ts_prvs$spp_name %in% "B. nigra"])} RW/GDD; Figure \ref{fig:tsPrvsYr}b; Table \ref{tab:tab_PrvsYr}) but in contrast, current year GDD increased growth for that species ($\beta =$ 
