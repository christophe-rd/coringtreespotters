...
($\beta =$ 0.15, UI:
0.02,
0.28 \RW scaled GDD) but the current year GDD did not change its growth, unlike the observed response when the previous year conditions were not considered. {\Ball}'s congeneric {\Bnig}, also grew less with previous year GDD ($\beta =$ 
-0.08, UI:
-0.17,
...
($\beta$: 1.04, UI:
0.39,
1.7 $\Delta$ EOS/SOS; Figure \ref{fig:SOSonEOS}. We did not find any clear difference on baseline growth across years (Figure \ref{fig:tsmuplotayear}; Table \ref{tab:tab_all}) and between species (Figure \ref{fig:tsxyGDDyr};\ref{fig:tsxyGSLyr}). Growth was similar across individual trees (Figure \ref{fig:tsmuplotatreeid}).
