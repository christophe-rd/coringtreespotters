...
1.04, UI:
0.39,
1.70 EOS/SOS; Figure \ref{fig:SOSonEOS}). We did not find any clear difference on baseline growth across years (Figure \ref{fig:tsmuplotayear}; Table \ref{tab:tab_all}) and between species (Figure \ref{fig:tsxyGDDyr}; \ref{fig:tsxyGSLyr}). Growth was similar across individual trees (Figure \ref{fig:tsmuplotatreeid}).
...
While our species share an array of growth strategies (see Table \ref{tab:tsCount}), we did not find a consistent growth trend across them. 

Only {\Ball} responded to the thermal season and its decrease in growth suggests that this species may be harmed by warmer seasons. This is further consequential with its negative response to a later end of season, when GDDs accumulate fast. With future warming, {\Ball} could be locally extirpated to its southernmost distribution because growth could not keep up with warmer and longer seasons.

In addition, the fast-growing {\Tame} \citep[Table \ref{tab:tsCount};][]{burns_silvics_1990} grew less with an earlier SOS---when the days are short and cool---despite our expectation of species with this life-history strategy that should allow them to have an edge by starting to grow earlier in the season \citep{loughnan_budburst_2025}. Ultimately, these findings suggest little difference between fast and slow-growth species in how their growth responds to the GS length and thermal conditions; this shifted slightly when considering also the previous year conditions. 
