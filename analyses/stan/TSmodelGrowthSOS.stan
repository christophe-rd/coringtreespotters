...
generated quantities {
  ...
  real atreeid_prior = abs(normal_rng(0, 2)) * zatreeid_prior;
  ...
  # Sim for each tree id, at each sosseq
  matrix[Nsosseq, Ntreeid] y_post;

  for (t in 1:Ntreeid) {
    for (g in 1:Nsosseq) {
      y_post[g, t] = normal_rng(fullintercept[t] + (treeid_slope[t]/ sosscale) * sosseq[g], sigma_y);
    }
  }
  
  # Sim for each species
  matrix[Nsosseq, Nspp] spp_mean;
  matrix[Nsosseq, Nspp] spp_post;

  spp_mean = rep_matrix(0, Nsosseq, Nspp);

  for (t in 1:Ntreeid) {
    int s = treeid_species[t];
    for (g in 1:Nsosseq) {
      spp_mean[g, s] += (fullintercept[t] + (treeid_slope[t] / sosscale) * sosseq[g])
                        / Ntreeid_per_spp[s];
    }
  }
